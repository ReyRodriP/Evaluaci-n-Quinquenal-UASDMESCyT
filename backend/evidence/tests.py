...
from evaluation.models import Periodo, Criterio, Indicador, Asignacion
from organization.models import Facultad, Departamento
from .models import EstadoEvidencia, Evidencia, VersionEvidencia, Observacion
...
def _make_asignacion(departamento=None):
    ...
    if not departamento:
        departamento = Departamento.objects.create(
            nombre='Depto Test', facultad=facultad
        )
    periodo = Periodo.objects.create(
        nombre='Periodo 2025', fecha_inicio='2025-01-01', fecha_fin='2025-12-31'
    )
    criterio = Criterio.objects.create(
        nombre='Criterio Test', periodo=periodo
    )
    indicador = Indicador.objects.create(
        nombre='Indicador Test', criterio=criterio
    )
    return Asignacion.objects.create(
        indicador=indicador, departamento=departamento, periodo=periodo
    )
...
class EvidenciaModelTests(TestCase):
    ...
    def test_estado_default(self):
        evidencia = Evidencia.objects.create(
            titulo='Test', descripcion='Test', asignacion=self.asignacion
        )
        self.assertEqual(evidencia.estado, EstadoEvidencia.ACTIVA)

    def test_str(self):
        evidencia = Evidencia.objects.create(
            titulo='Mi Evidencia', descripcion='Test', asignacion=self.asignacion
        )
...
class VersionEvidenciaModelTests(TestCase):
    def setUp(self):
        ...
        self.evidencia = Evidencia.objects.create(
            titulo='Evidencia V', descripcion='Test', asignacion=self.asignacion
        )
    ...
    def test_str(self):
        version = VersionEvidencia.objects.create(
            evidencia=self.evidencia, archivo='evidencias/test.pdf'
        )
...
class ObservacionModelTests(TestCase):
    def setUp(self):
        self.asignacion = _make_asignacion()
        self.evidencia = Evidencia.objects.create(
            titulo='Evidencia O', descripcion='Test', asignacion=self.asignacion
        )
    ...
    def test_activo_default(self):
        obs = Observacion.objects.create(
            version=self.version, usuario=self.user, comentario='Test'
        )
...
class EvidenciaViewSetTests(TestCase):
    ...
    def test_create_evidencia(self):
        ...
        self.assertTrue(
            Evidencia.objects.filter(titulo='Nueva Evidencia').exists()
        )
