...
class FixedPermissionsGroupAdmin(BaseGroupAdmin):
    ...
    def permissions_status(self, obj):
        if obj.name not in ROLE_PERMISSIONS:
            return format_html('<span style="color:orange;">Rol libre</span>')
        extra, missing = get_role_permissions_diff(obj)
        if not extra and not missing:
            return format_html('<span style="color:green;">✓ Sincronizado</span>')
        parts = []
        if extra:
            parts.append(f'<span style="color:red;">{len(extra)} extra</span>')
        if missing:
            parts.append(f'<span style="color:orange;">{len(missing)} faltante</span>')
