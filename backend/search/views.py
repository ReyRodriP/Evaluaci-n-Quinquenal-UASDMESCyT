...
def search(request):
    ...
    indicadores = restringir_indicador(Indicador.objects.filter(activo=True)).filter(
        nombre__icontains=q
    ) | restringir_indicador(Indicador.objects.filter(activo=True)).filter(descripcion__icontains=q)
    indicadores_data = [
    ...
    departamentos_data = [
        {"id": d.pk, "nombre": d.nombre, "facultad": d.facultad.nombre, "tipo": "Departamento"} for d in deptos_qs[:10]
    ]
