...
def _filas_observaciones(qs):
    conteos = dict(qs.values_list("version__evidencia_id").annotate(total=Count("id")).order_by())
...
def _build_pdf(titulo, columnas, filas):
    ...
    estilo_subtitulo = ParagraphStyle(
        "subtitulo_reporte", parent=estilos["Normal"], fontSize=9, alignment=1, spaceAfter=10
    )
    ...
    estilo_cabecera = ParagraphStyle(
        "cabecera_reporte", parent=estilos["Normal"], fontSize=8, textColor=colors.white, alignment=1
    )
...
def _responsable_departamento(departamento):
    perfiles = (
        PerfilUsuario.objects.filter(departamento=departamento)
...
def _data_por_departamento(request, pk):
    ...
    for asignacion in asignaciones:
        ultima_version = (
            VersionEvidencia.objects.filter(evidencia__asignacion=asignacion).order_by("-fecha_subida").first()
        )

        ultimo_historial = HistorialEstado.objects.filter(asignacion=asignacion).order_by("-fecha").first()

        filas.append(
            {
                "indicador": asignacion.indicador.nombre,
                "estado": asignacion.estado,
                "fecha_modificacion": (_formato_fecha(ultimo_historial.fecha) if ultimo_historial else ""),
