...
from .models import (
    Periodo, Criterio, Indicador, Asignacion, EstadoAsignacion, HistorialEstado,
)
...
class PeriodoModelTests(TestCase):
    def test_crear_periodo(self):
        periodo = Periodo.objects.create(
            nombre="Evaluacion 2026", fecha_inicio=date(2026, 1, 1),
    ...
    def test_str_representation(self):
        periodo = Periodo.objects.create(
            nombre="Periodo Test", fecha_inicio=date(2026, 1, 1),
    ...
    def test_periodo_default_activo(self):
        periodo = Periodo.objects.create(
            nombre="SinActivo", fecha_inicio=date(2026, 1, 1),
            fecha_fin=date(2026, 6, 30),
        )
...
class CriterioModelTests(TestCase):
    def setUp(self):
        self.periodo = Periodo.objects.create(
            nombre="P1", fecha_inicio=date(2026, 1, 1),
            fecha_fin=date(2026, 6, 30),
        )

    def test_crear_criterio(self):
        criterio = Criterio.objects.create(
            nombre="Calidad", descripcion="Criterio de calidad",
        )
    ...
    def test_criterio_con_periodo(self):
        criterio = Criterio.objects.create(
            nombre="Pertinencia", descripcion="Criterio pertinente",
    ...
    def test_str_representation(self):
        criterio = Criterio.objects.create(
            nombre="Impacto", periodo=self.periodo,
        )
...
class IndicadorModelTests(TestCase):
    def setUp(self):
        ...
        self.criterio = Criterio.objects.create(
            nombre="C1", periodo=self.periodo,
        )

    def test_crear_indicador(self):
        indicador = Indicador.objects.create(
            nombre="Ind-A", descripcion="Desc", criterio=self.criterio,
        )
    ...
    def test_indicador_obligatorio_default(self):
        indicador = Indicador.objects.create(
            nombre="Ind-B", criterio=self.criterio,
        )
        self.assertFalse(indicador.obligatorio)

    def test_str_representation(self):
        indicador = Indicador.objects.create(
            nombre="Ind-Test", criterio=self.criterio,
        )
...
class AsignacionModelTests(TestCase):
    def setUp(self):
        ...
        self.indicador = Indicador.objects.create(
            nombre="I1", criterio=self.criterio,
        )
        self.facultad = Facultad.objects.create(nombre="Facultad X")
        self.departamento = Departamento.objects.create(
            nombre="Depto Y", facultad=self.facultad,
        )

    def test_crear_asignacion(self):
        asignacion = Asignacion.objects.create(
            indicador=self.indicador, departamento=self.departamento,
            periodo=self.periodo,
        )
...
class HistorialEstadoModelTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username="user_h", password="pass123", email="h@test.com",
        )
        self.periodo = Periodo.objects.create(
            nombre="P1", fecha_inicio=date(2026, 1, 1),
            fecha_fin=date(2026, 6, 30),
    ...
    def test_historial_orden_fecha(self):
        ...
        historial = list(
            HistorialEstado.objects.filter(asignacion=self.asignacion)
        )
    ...
    def test_str_representation(self):
        ...
        expected = (
            f"{self.asignacion} "
            f"{EstadoAsignacion.PENDIENTE} → {EstadoAsignacion.EN_PROGRESO}"
        )
...
class PeriodoViewSetTests(TestCase):
    def setUp(self):
        ...
        self.admin_user = User.objects.create_user(
            username="admin_p", password="admin123", email="admin_p@test.com",
        )
        self.admin_user.groups.add(self.admin_group)
        self.admin_token = Token.objects.create(user=self.admin_user)

        self.consulta_user = User.objects.create_user(
            username="consulta_p", password="consulta123",
...
class AsignacionViewSetTests(TestCase):
    def setUp(self):
        ...
        self.admin_user = User.objects.create_user(
            username="admin_a", password="admin123", email="admin_a@test.com",
        )
        self.admin_user.groups.add(self.admin_group)
        self.admin_token = Token.objects.create(user=self.admin_user)
    ...
    def test_create_asignacion_admin(self):
        ...
        response = self.client.post(
            "/api/asignaciones/", payload, format="json",
        )
        self.assertEqual(response.status_code, 201)
        self.assertTrue(
