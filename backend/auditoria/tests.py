...
class AuditoriaModelTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            username='auditor', email='aud@test.com', password='testpass123'
        )
    ...
    def test_str(self):
        auditoria = Auditoria.objects.create(
            ...
            accion='Eliminar',
            modelo='Evidencia',
            descripcion='Eliminación de prueba'
        )
    ...
    def test_orden_fecha(self):
        ...
        a1 = Auditoria.objects.create(
            usuario=self.user, accion='Primera', modelo='Test',
            descripcion='Primera acción', fecha=now
        )
        a2 = Auditoria.objects.create(
            usuario=self.user, accion='Segunda', modelo='Test',
            descripcion='Segunda acción', fecha=now
        )
...
class AuditoriaViewSetTests(TestCase):
    def setUp(self):
        ...
        self.admin_user = User.objects.create_superuser(
            username='admin_aud', email='admin_aud@test.com',
    ...
    def test_list_admin(self):
        Auditoria.objects.create(
            usuario=self.admin_user, accion='Test', modelo='Evidencia',
            descripcion='Test admin access'
        )
